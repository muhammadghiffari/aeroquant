...
### 5.3 LLM Provider & Orchestration Decision — updated in 2.6

**Primary decision (adopted 2026-08-31): BluePack as the primary Anthropic-compatible provider.**

- **What BluePack is:** BluePack (`https://ai.bluepack.my.id/anthropic`) is an independently-operated Anthropic-compatible API endpoint. It is **not** an Anthropic-owned or Anthropic-affiliated service. The team has verified it passes a four-part smoke test (direct Anthropic-compatible request, tool calling, structured output, and three-policy `model_gateway.py` test) on 2026-08-31. No billing, ToS-equivalence, or model-parity claims are made beyond what has been directly verified.
- **How it is used:** `ANTHROPIC_BASE_URL=https://ai.bluepack.my.id/anthropic` with a standard `ANTHROPIC_API_KEY`. The existing `_anthropic()` client builder in `model_gateway.py` already reads `ANTHROPIC_BASE_URL` from the environment, so no code change is required to adopt this endpoint.
- **Approval date:** 2026-08-31. Any additional endpoints require an explicit team decision documented here before use.
- **Claude Code subscription OAuth routing: still prohibited.** Routing through a Claude Code or ChatGPT-Plus subscription token (whether via official or proxy endpoints) remains a ToS violation and a closed decision. This prohibition is unaffected by the BluePack adoption.

**Fallback provider: Featherless.ai (unchanged from 2.5).**
- Featherless remains the confirmed secondary/fallback provider, wired via `model_gateway.py`'s circuit-breaker + fallback-chain design. Qwen3-32B is the active default. GLM-4.7-Flash / GLM-5.2 remain a candidate swap under evaluation — see the GLM evaluation notes in 2.6's version note and do not switch until the corrected smoke test passes.
- BytePlus Ark remains explicitly deferred (no partner/subscription evidence) and is not in the active fallback chain.

**Strategic caveat on provider mix (carried forward):** `critic`/`strong_reasoning` gate real order placement. Auto-fallback to Featherless during exactly the moments the primary is degraded (which may correlate with high market volatility) is a quality trade-off, not a free reliability win. The fallback chain handles this automatically, but the team should be aware of it in live operation.

**Orchestration — LangGraph, not a chat-CLI agent.** *(Unchanged from 2.4.)* A vendor-native Claude Agent SDK option was considered and rejected for the core trading loop (Claude-only by design, built around a single agent, not a multi-role checkpointed graph). The Claude Code subscription OAuth routing prohibition (raised and rejected in 2.3) remains closed and is unaffected by the BluePack adoption.
